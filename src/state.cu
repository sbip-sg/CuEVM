    ...
    namespace state {
        ...
        __host__ __device__ state_t::state_t(const state_t &other) {
            ...
            if (no_accounts > 0) {
                ...
                for (size_t idx = 0; idx < no_accounts; idx++) {
        ...
        __host__ cJSON* state_t::to_json(
            ...
            if (flags != nullptr) {
                flag_string_ptr = new char[sizeof(uint32_t) * 2 + 3];
            }
...
public:
    ...
    */
    ...
    )
    {
        ...
        {
            bytes_string = cuEVM::byte_array::hex_from_bytes(account.bytecode, account.code_size);
