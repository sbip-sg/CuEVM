...
public:
    ...
    */
    ...
    )
    {
        ...
        kernel_accessed_state_S1<params><<<1, count>>>(tmp_gpu_instances, gpu_instances, count);
        ...
        kernel_accessed_state_S2<params><<<1, count>>>(tmp_gpu_instances, gpu_instances, count);
        CUDA_CHECK(cudaDeviceSynchronize());
...
public:
    ...
    */
    ...
    )
    {
        ...
        // run the second kernel which copy the bytecode and storage
        kernel_touch_state_S2<params><<<1, count>>>(tmp_gpu_instances, gpu_instances, count);
        CUDA_CHECK(cudaDeviceSynchronize());
...
public:
    ...
    */
    ...
    ) : _arith(arith)
    {
        ...
        ONE_THREAD_PER_INSTANCE(
            ...
            tmp_content->logs = new log_data_t[_allocated_size];
        )
        _content = tmp_content;
    }
    ...
    __host__ __device__ __forceinline__ ~log_state_t()
    {
        ONE_THREAD_PER_INSTANCE(
            if (_content != NULL)
            {
                ...
                {
                    if (_content->logs->record.size > 0)
                    {
                        delete[] _content->logs->record.data;
                        _content->logs->record.data = NULL;
                        _content->logs->record.size = 0;
                    }
    ...
    */
    ...
    )
    {
        ...
        // run the first kernel which copy the accoutns details and read operations
        kernel_log_state_S1<params><<<1, count>>>(tmp_gpu_instances, gpu_instances, count);
        CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaFree(gpu_instances));
        ...
        // run the second kernel which copy the bytecode and storage
        kernel_log_state_S2<params><<<1, count>>>(tmp_gpu_instances, gpu_instances, count);
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    */
     ...
     * @param[in] arith The arithemtic instance
     * @param[in] log_state_data The lof state data
