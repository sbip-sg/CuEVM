...
class gpu_fixed_storage_t {
  ...
  //constructor
  __device__ __forceinline__ gpu_fixed_storage_t(arith_env_t<params> arith, fixed_storage_data_t *content) : _arith(arith), _content(content) {
  }
  ...
  __device__ __forceinline__ uint32_t get(const bn_t &address, const bn_t &key, bn_t &value) {
    ...
    for (size_t idx = 0; idx<_content->_size; idx++) {
  ...
  __device__ __forceinline__ uint32_t set(const bn_t &address, const bn_t &key, const bn_t &value) {
    ...
    if (_content->_size == params::STORAGE_SIZE)
      return 1;
    cgbn_store(_arith._env, &(_content->address[_content->_size]), address);
    cgbn_store(_arith._env, &(_content->key[_content->_size]), key);
    cgbn_store(_arith._env, &(_content->value[_content->_size]), value);
    _content->_size++;
    return 0;
  }
  ...
  __host__ static void free_storage_data(fixed_storage_data_t *cpu_instances, uint32_t count) {
    free(cpu_instances);
  }
  // free the gpu storage structure info on the host
  __host__ static void free_gpu_storage_data(fixed_storage_data_t *gpu_instances, uint32_t count) {
    cudaFree(gpu_instances);
  }

  __host__ static void write_storage(FILE *fp, fixed_storage_data_t *cpu_instances, uint32_t count) {
    for(uint32_t idx=0; idx<count; idx++) {
      fprintf(fp, "INSTACE: %08x , STORAGE_SIZE: %lu , STORAGE_DATA: ", idx, cpu_instances[idx]._size);
      fprintf(fp, "\n");
      for(uint32_t jdx=0; jdx<cpu_instances[idx]._size; jdx++) {
        fprintf(fp, "ADDRESS- ");
        for(uint32_t kdx=0; kdx<params::BITS/32; kdx++) {
          fprintf(fp, "%08x ", cpu_instances[idx].address[jdx]._limbs[kdx]);
        }
        fprintf(fp, "KEY- ");
        for(uint32_t kdx=0; kdx<params::BITS/32; kdx++) {
          fprintf(fp, "%08x ", cpu_instances[idx].key[jdx]._limbs[kdx]);
        }
        fprintf(fp, "VALUE- ");
        for(uint32_t kdx=0; kdx<params::BITS/32; kdx++) {
          fprintf(fp, "%08x ", cpu_instances[idx].value[jdx]._limbs[kdx]);
        }
        fprintf(fp, "\n");
      }
      fprintf(fp, "\n");
