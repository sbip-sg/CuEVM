...
class arith_env_t {
   public:
     ...
     */
    __device__ __forceinline__ arith_env_t(cgbn_monitor_t monitor, cgbn_error_report_t *report, uint32_t instance)
        : _context(monitor, report, instance), _env(_context), _instance(instance) {}
     ...
     * @param[in] instance The instance number for the CGBN library
     */
    __host__ __forceinline__ arith_env_t(cgbn_monitor_t monitor, uint32_t instance)
        : _context(monitor), _env(_context), _instance(instance) {}
     ...
     */
    __host__ __device__ __forceinline__ arith_env_t(const arith_env_t &env)
        : _context(env._context), _env(_context), _instance(env._instance) {}

    __host__ __device__ __forceinline__ void address_conversion(bn_t &address) {
     ...
     */
    __host__ __device__ __forceinline__ size_t memory_from_cgbn(uint8_t *dst, bn_t &src) {
     ...
     */
    __host__ __device__ __forceinline__ void cgbn_from_memory(bn_t &dst, uint8_t *src) {
        for (uint32_t idx = 0; idx < BYTES; idx++) {
     ...
     */
    __host__ __device__ __forceinline__ void cgbn_from_size_t(bn_t &dst, size_t src) {
     ...
     * @return 1 for overflow, 0 otherwiese
     */
    __host__ __device__ __forceinline__ int32_t uint64_t_from_cgbn(uint64_t &dst, bn_t &src) {
        bn_t MAX_uint64_t;
     ...
     */
    __host__ void hex_string_from_cgbn_memory(char *dst_hex_string, evm_word_t &src_cgbn_mem, uint32_t count = LIMBS) {
        dst_hex_string[0] = '0';
        dst_hex_string[1] = 'x';
        for (uint32_t idx = 0; idx < count; idx++) {
            sprintf(dst_hex_string + 2 + idx * 8, "%08x", src_cgbn_mem._limbs[count - 1 - idx]);
        }
     ...
     * @param[in] count The number of limbs
     */
    __host__ void pretty_hex_string_from_cgbn_memory(char *dst_hex_string, evm_word_t &src_cgbn_mem,
     ...
     * @return 1 for overflow, 0 otherwiese
     */
    __host__ int32_t cgbn_memory_from_hex_string(evm_word_t &dst_cgbn_memory, char *src_hex_string) {
        mpz_t value;
        size_t written;
        mpz_init(value);
        if ((src_hex_string[0] == '0') && ((src_hex_string[1] == 'x') || (src_hex_string[1] == 'X'))) {
        ...
        mpz_export(dst_cgbn_memory._limbs, &written, -1, sizeof(uint32_t), 0, 0, value);
        while (written < LIMBS) {
     ...
     */
    __host__ __device__ __forceinline__ void cgbn_memory_from_size_t(evm_word_t &dst_cgbn_memory, size_t src) {
        bn_t src_cgbn;
     ...
     */
    __host__ __device__ __forceinline__ void print_cgbn_memory(evm_word_t &src_cgbn_memory) {
        for (uint32_t idx = 0; idx < LIMBS; idx++) printf("%08x ", src_cgbn_memory._limbs[LIMBS - 1 - idx]);
        printf("\n");
    }
     ...
     */
    __host__ __device__ __forceinline__ int32_t has_gas(bn_t &gas_limit, bn_t &gas_used, uint32_t &error_code) {
     ...
     * @param[in] gas_used The gas used
     */
    __host__ __device__ __forceinline__ void max_gas_call(bn_t &gas_capped, bn_t &gas_limit, bn_t &gas_used) {
     ...
     */
    __host__ __device__ __forceinline__ uint8_t *get_data(data_content_t &data_content, bn_t &index, bn_t &length,
                                                          size_t &available_size) {
        available_size = 0;
        size_t index_s;
        int32_t overflow = size_t_from_cgbn(index_s, index);
        if ((overflow != 0) || (index_s >= data_content.size)) {
            return NULL;
        } else {
            size_t length_s;
            overflow = size_t_from_cgbn(length_s, length);
            if ((overflow != 0) || (length_s > data_content.size - index_s)) {
     ...
     */
    __host__ __device__ __forceinline__ void evm_words_gas_cost(bn_t &gas_used, bn_t &length, uint32_t gas_per_word) {
     ...
     */
    __host__ __device__ __forceinline__ void initcode_cost(bn_t &gas_used, bn_t &initcode_length) {
     ...
     */
    __host__ __device__ __forceinline__ void keccak_cost(bn_t &gas_used, bn_t &length) {
     ...
     * @param[in] length The length of the data in bytes
     */
    __host__ __device__ __forceinline__ void memory_cost(bn_t &gas_used, bn_t &length) {
     ...
     * @param[in] length The length of the data in bytes
     */
    __host__ __device__ __forceinline__ void sha256_cost(bn_t &gas_used, bn_t &length) {
