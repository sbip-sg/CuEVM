...
__host__ __device__ int32_t state_t::is_empty_account(ArithEnv &arith, const bn_t &address) {
    int32_t error_code;
    uint32_t index;
    error_code = get_account_index(arith, address, index);
    return (error_code == ERROR_SUCCESS) ? accounts[index].is_empty() : error_code;
}
