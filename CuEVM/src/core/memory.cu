...
__host__ __device__ void evm_memory_t::get_memory_cost(ArithEnv &arith, bn_t &cost) const {
    cgbn_load(arith.env, cost, (CuEVM::cgbn_evm_word_t_ptr)&memory_cost);
}

__host__ __device__ void evm_memory_t::increase_memory_cost(ArithEnv &arith, const bn_t &memory_expansion_cost) {
    bn_t cuurent_cost;
    cgbn_load(arith.env, cuurent_cost, (CuEVM::cgbn_evm_word_t_ptr)&memory_cost);
    cgbn_add(arith.env, cuurent_cost, cuurent_cost, memory_expansion_cost);
    cgbn_store(arith.env, (CuEVM::cgbn_evm_word_t_ptr)&memory_cost, cuurent_cost);
}
...
__host__ __device__ int32_t evm_memory_t::get_last_offset(ArithEnv &arith, const bn_t &index, const bn_t &length,
...
__host__ __device__ int32_t evm_memory_t::grow(ArithEnv &arith, const bn_t &index, const bn_t &length) {
    uint32_t offset;
...
__host__ __device__ int32_t evm_memory_t::get(ArithEnv &arith, const bn_t &index, const bn_t &length,
...
__host__ __device__ int32_t evm_memory_t::set(ArithEnv &arith, const CuEVM::byte_array_t &data, const bn_t &index,
                                              const bn_t &length) {
    int32_t error_code = ERROR_SUCCESS;
    error_code = (cgbn_compare_ui32(arith.env, length, 0) < 0) ? ERR_MEMORY_INVALID_SIZE : error_code;
    error_code |= grow(arith, index, length);
...
__global__ void transfer_kernel(evm_memory_t *dst_instances, evm_memory_t *src_instances, uint32_t instance_count) {
    ...
    if (src_instances[instance].data.size > 0) {
        memcpy(dst_instances[instance].data.data, src_instances[instance].data.data,
...
__host__ evm_memory_t *get_gpu_from_cpu(evm_memory_t *cpu_instances, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        if (cpu_instances[i].data.size > 0) {
            ...
            CUDA_CHECK(cudaMemcpy(tmp_cpu_instances[i].data.data, cpu_instances[i].data.data,
                                  cpu_instances[i].data.size, cudaMemcpyHostToDevice));
        }
    }
...
__host__ evm_memory_t *get_cpu_from_gpu(evm_memory_t *gpu_instances, uint32_t count) {
    ...
    for (uint32_t idx = 0; idx < count; idx++) {
        if (tmp_cpu_instances[idx].size > 0) {
            CUDA_CHECK(cudaMalloc((void **)&tmp_cpu_instances[idx].data.data,
    ...
    CUDA_CHECK(cudaMemcpy(cpu_instances, gpu_instances, sizeof(evm_memory_t) * count, cudaMemcpyDeviceToHost));
    tmp_cpu_instances = new evm_memory_t[count];
    std::copy(cpu_instances, cpu_instances + count, tmp_cpu_instances);
    for (size_t idx = 0; idx < count; idx++) {
        if (tmp_cpu_instances[idx].data.size > 0) {
            tmp_cpu_instances[idx].data.data = new uint8_t[tmp_cpu_instances[idx].data.size];
            CUDA_CHECK(cudaMemcpy(tmp_cpu_instances[idx].data.data, cpu_instances[idx].data.data,
                                  sizeof(uint8_t) * tmp_cpu_instances[idx].data.size, cudaMemcpyDeviceToHost));
        } else {
            tmp_cpu_instances[idx].data.data = nullptr;
        }
    }
