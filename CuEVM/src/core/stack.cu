...
__host__ __device__ evm_stack_t::evm_stack_t()
    : stack_base(nullptr), stack_offset(0), capacity(0) {}
...
__host__ __device__ evm_stack_t &evm_stack_t::operator=(
...
__host__ __device__ int32_t evm_stack_t::push(ArithEnv &arith,
                                              const bn_t &value) {
...
__host__ __device__ int32_t evm_stack_t::pushx(ArithEnv &arith, uint8_t x,
    ...
    for (uint8_t idx = (x - src_byte_size); idx < x; idx++) {
        cgbn_insert_bits_ui32(arith.env, r, r, idx * 8, 8,
                              src_byte_data[x - 1 - idx]);
    }
    return push(arith, r);
}

__host__ __device__ int32_t evm_stack_t::get_index(ArithEnv &arith,
                                                   uint32_t index, bn_t &y) {
    if (index > size()) {
...
__host__ __device__ int32_t evm_stack_t::dupx(ArithEnv &arith, uint32_t x) {
    bn_t r;
    int32_t error_code = ((x > 16) || (x < 1)) ? ERROR_STACK_INVALID_SIZE
                                               : get_index(arith, x, r);
...
__host__ void evm_stack_t::cpu_free(evm_stack_t *instances, uint32_t count) {
    delete[] instances;
}
__host__ evm_stack_t *evm_stack_t::gpu_from_cpu(evm_stack_t *cpu_instances,
    ...
    for (uint32_t idx = 0; idx < count; idx++) {
        if (cpu_instances[idx].stack_base != nullptr) {
            CUDA_CHECK(
                cudaMalloc(&tmp_gpu_instances[idx].stack_base,
                           cpu_instances[idx].capacity * sizeof(evm_word_t)));
            CUDA_CHECK(
                cudaMemcpy(tmp_gpu_instances[idx].stack_base,
    ...
    CUDA_CHECK(cudaMemcpy(gpu_instances, tmp_gpu_instances,
                          count * sizeof(evm_stack_t), cudaMemcpyHostToDevice));
    for (uint32_t idx = 0; idx < count; idx++) {
...
__host__ void evm_stack_t::gpu_free(evm_stack_t *gpu_instances,
                                    uint32_t count) {
    evm_stack_t *tmp_gpu_instances = new evm_stack_t[count];
    CUDA_CHECK(cudaMemcpy(tmp_gpu_instances, gpu_instances,
...
__host__ evm_stack_t *evm_stack_t::cpu_from_gpu(evm_stack_t *gpu_instances,
    ...
    transfer_kernel_evm_stack_t<<<count, 1>>>(tmp_gpu_instances, gpu_instances,
                                              count);
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaFree(gpu_instances));

    CUDA_CHECK(cudaMemcpy(tmp_cpu_instances, tmp_gpu_instances,
                          sizeof(evm_stack_t) * count, cudaMemcpyDeviceToHost));

    for (uint32_t idx = 0; idx < count; idx++) {
        if (tmp_cpu_instances[idx].stack_offset > 0) {
            cpu_instances[idx].stack_base =
                new evm_word_t[tmp_cpu_instances[idx].stack_offset];
            CUDA_CHECK(cudaMemcpy(
                cpu_instances[idx].stack_base,
                tmp_cpu_instances[idx].stack_base,
                sizeof(evm_word_t) * tmp_cpu_instances[idx].stack_offset,
...
__global__ void transfer_kernel_evm_stack_t(evm_stack_t *dst, evm_stack_t *src,
    ...
    memcpy(dst[instance].stack_base, src[instance].stack_base,
