...
__host__ __device__ int32_t get_contract_address_create(
    ...
    cgbn_store(arith.env, (cgbn_evm_word_t_ptr)(&sender_address_word),
               sender_address);
    ...
    for (uint32_t idx = 0; idx < CuEVM::address_size; idx++) {
        rlp_list[2 + idx] =
            sender_address_bytes
                .data[CuEVM::word_size - CuEVM::address_size + idx];
    }
    ...
    if (cgbn_compare_ui32(arith.env, sender_nonce, 128) < 0) {
        ...
        if (cgbn_compare_ui32(arith.env, sender_nonce, 0) == 0) {
            rlp_list[2 + CuEVM::address_size] =
                0x80;  // special case for nonce 0
        } else {
            rlp_list[2 + CuEVM::address_size] =
                sender_nonce_bytes.data[CuEVM::word_size - 1];
        }
    } else {
        ...
        for (uint8_t idx = 0; idx < nonce_bytes; idx++) {
            rlp_list[2 + CuEVM::address_size + 1 + idx] =
                sender_nonce_bytes.data[CuEVM::word_size - nonce_bytes + idx];
        }
    }
...
__host__ __device__ int32_t get_contract_address_create2(
    ...
    uint32_t total_bytes =
        1 + CuEVM::address_size + CuEVM::word_size + CuEVM::hash_size;

    CuEVM::byte_array_t hash_code(CuEVM::hash_size);
    CuCrypto::keccak::sha3(init_code.data, init_code.size, hash_code.data,
                           CuEVM::hash_size);

    CuEVM::byte_array_t input_data(total_bytes);
    input_data.data[0] = 0xff;
    for (uint32_t idx = 0; idx < CuEVM::address_size; idx++) {
        input_data.data[1 + idx] =
            sender_address_bytes
                .data[CuEVM::word_size - CuEVM::address_size + idx];
    }
    for (uint32_t idx = 0; idx < CuEVM::word_size; idx++) {
        input_data.data[1 + CuEVM::address_size + idx] =
            salt_bytes.data[CuEVM::word_size - CuEVM::word_size + idx];
    }
    for (uint32_t idx = 0; idx < CuEVM::hash_size; idx++) {
        input_data.data[1 + CuEVM::address_size + CuEVM::word_size + idx] =
            hash_code.data[idx];
    }
