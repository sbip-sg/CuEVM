...
__host__ __device__ int32_t has_gas(ArithEnv &arith, const bn_t &gas_limit,
                                    const bn_t &gas_used) {
    return (cgbn_compare(arith.env, gas_limit, gas_used) < 0)
...
__host__ __device__ void initcode_cost(ArithEnv &arith, bn_t &gas_used,
    ...
    evm_words_gas_cost(arith, gas_used, initcode_length,
                       GAS_INITCODE_WORD_COST);
}
        ...
        if (cgbn_compare(arith.env, current_value, original_value) == 0) {
            if (cgbn_compare_ui32(arith.env, original_value, 0) == 0) {
                ...
                if (cgbn_compare_ui32(arith.env, new_value, 0) == 0) {
                    cgbn_add_ui32(arith.env, gas_refund, gas_refund,
                                  GAS_SSTORE_CLEARS_SCHEDULE);
                }
            }
        } else {
            cgbn_add_ui32(arith.env, gas_used, gas_used, GAS_SLOAD);
            if (cgbn_compare_ui32(arith.env, original_value, 0) != 0) {
                if (cgbn_compare_ui32(arith.env, current_value, 0) == 0) {
                    cgbn_sub_ui32(arith.env, gas_refund, gas_refund,
            ...
            if (cgbn_compare(arith.env, original_value, new_value) == 0) {
                if (cgbn_compare_ui32(arith.env, original_value, 0) == 0) {
                    cgbn_add_ui32(arith.env, gas_refund, gas_refund,
                                  GAS_STORAGE_SET - GAS_SLOAD);
                } else {
                    cgbn_add_ui32(arith.env, gas_refund, gas_refund,
                                  GAS_STORAGE_RESET - GAS_SLOAD);
                }
            }
        }
    }
...
__host__ __device__ int32_t transaction_intrinsic_gas(
    ...
    if (transaction.type == 1) {
        ...
        for (uint32_t idx = 0; idx < transaction.access_list.accounts_count;
             idx++) {
            cgbn_add_ui32(arith.env, gas_intrinsic, gas_intrinsic,
                          GAS_ACCESS_LIST_ADDRESS);
            cgbn_add_ui32(
                arith.env, gas_intrinsic, gas_intrinsic,
                GAS_ACCESS_LIST_STORAGE *
                    transaction.access_list.accounts[idx].storage_keys_count);
        }
    }
...
__host__ __device__ int32_t memory_grow_cost(
    ...
    do {
        ...
        if (cgbn_get_uint32_t(arith.env, offset_ui32, offset) ==
        ...
        cgbn_mul_wide(arith.env, memory_size_word_wide, memory_size_word,
                      memory_size_word);
        ...
        if (cgbn_sub(arith.env, memory_expansion_cost, memory_cost,
                     old_memory_cost) != 0) {
            cgbn_set_ui32(arith.env, memory_expansion_cost, 0);
        }
        if (cgbn_add(arith.env, gas_used, gas_used, memory_expansion_cost) !=
            0) {
            break;
        }
