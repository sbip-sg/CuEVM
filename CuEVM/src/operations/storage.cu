...
__host__ __device__ int32_t SLOAD(ArithEnv &arith, const bn_t &gas_limit,
    ...
    error_code |= CuEVM::gas_cost::sload_cost(arith, gas_used, touch_state,
                                              storage_address, key);
...
__host__ __device__ int32_t SSTORE(ArithEnv &arith, const bn_t &gas_limit,
    ...
    int32_t error_code =
        (message.get_static_env() ? ERROR_STATIC_CALL_CONTEXT_SSTORE
                                  : ERROR_SUCCESS);
    // cgbn_add_ui32(arith.env, gas_used, gas_used, GAS_ZERO);
    bn_t gas_left;
    cgbn_sub(arith.env, gas_left, gas_limit, gas_used);
    error_code |= (cgbn_compare_ui32(arith.env, gas_left, GAS_STIPEND) < 0
                       ? ERROR_OUT_OF_GAS
                       : error_code);
    if (error_code != ERROR_SUCCESS) {
        return error_code;
