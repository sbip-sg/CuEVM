...
__device__ int32_t LOGX(const CuEVM::gas_t &gas_limit, CuEVM::gas_t &gas_used, CuEVM::evm_stack_t &stack,
    ...
    if (error_code == ERROR_SUCCESS) {
        ...
        error_code |= CuEVM::gas_cost::memory_grow_cost(&memory, memory_offset_ui32, length_ui32, memory_expansion_cost,
                                                        gas_used);
        ...
        if (error_code == ERROR_SUCCESS) {
            ...
            error_code |= memory.get(memory_offset_ui32, length_ui32, record.data);
