...
__host__ __device__ int32_t LOGX(ArithEnv &arith, const bn_t &gas_limit, bn_t &gas_used, CuEVM::evm_stack_t &stack,
    ...
    CuEVM::gas_cost::log_record_cost(arith, gas_used, length);

    CuEVM::gas_cost::log_topics_cost(arith, gas_used, no_topics);
    ...
    if (error_code == ERROR_SUCCESS) {
        ...
        if (error_code == ERROR_SUCCESS) {
            ...
            error_code |= memory.get(arith, memory_offset, length, record);

            bn_t address;
            message.get_contract_address(arith, address);
#ifdef ENABLE_LOGS
            log_state.push(arith, address, record, topics[0], topics[1], topics[2], topics[3], no_topics);
#endif
        }
    }
    return error_code;
