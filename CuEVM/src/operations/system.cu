...
generic_CALL(ArithEnv &arith, const bn_t &args_offset, const bn_t &args_size,
    ...
    error_code |= CuEVM::gas_cost::memory_grow_cost(
        arith, *current_state.memory_ptr, args_offset, args_size,
    ...
    if (cgbn_compare(arith.env, memory_expansion_cost_args,
                     memory_expansion_cost_ret) > 0) {
    ...
    printf("generic _ call memory_expansion_cost\n");
    ...
    if (new_state_ptr->message_ptr->get_call_type() != OP_DELEGATECALL) {
        if (cgbn_compare_ui32(arith.env, value, 0) > 0) {
            cgbn_add_ui32(arith.env, current_state.gas_used,
                          current_state.gas_used, GAS_CALL_VALUE);
    ...
    bn_t gas_capped;
    CuEVM::gas_cost::max_gas_call(arith, gas_capped, current_state.gas_limit,
    ...
    cgbn_add(arith.env, new_state_ptr->gas_limit, new_state_ptr->gas_limit,
             gas_stippend);

    error_code |= CuEVM::gas_cost::has_gas(arith, current_state.gas_limit,
                                           current_state.gas_used);
    printf("gas used generic_CALL\n");
    print_bnt(arith, current_state.gas_used);
    printf("gas limit generic_CALL\n");
    print_bnt(arith, new_state_ptr->gas_limit);
    if (error_code == ERROR_SUCCESS) {
        ...
        error_code |= current_state.memory_ptr->get(
            arith, args_offset, args_size, new_state_ptr->message_ptr->data);
    }
    return error_code;
}
...
generic_CREATE(ArithEnv &arith, CuEVM::evm_call_state_t &current_state,
    ...
    bn_t memory_expansion_cost;
    error_code |= CuEVM::gas_cost::memory_grow_cost(
        arith, *current_state.memory_ptr, memory_offset, length,
    ...
    if (error_code == ERROR_SUCCESS) {
        ...
        current_state.memory_ptr->get(arith, memory_offset, length,
                                      initialisation_code);
        ...
        CuEVM::gas_cost::max_gas_call(
            arith, gas_capped, current_state.gas_limit, current_state.gas_used);
        ...
        new_state_ptr = new CuEVM::evm_call_state_t(
            ...
            new CuEVM::evm_message_call_t(
                arith, sender_address, contract_address, contract_address,
                gas_capped, value, current_state.message_ptr->get_depth() + 1,
                opcode, contract_address, call_data, initialisation_code,
                ret_offset, ret_size,
        ...
        printf("is warm account new_state_ptr %d\n",
               new_state_ptr->touch_state.is_warm_account(arith,
                                                          contract_address));
        printf(
            "is warm account current_state %d\n",
...
__host__ __device__ int32_t CALL(ArithEnv &arith,
    ...
    if (error_code == ERROR_SUCCESS) {
        ...
        new_state_ptr = new CuEVM::evm_call_state_t(
            arith, &current_state,
            new CuEVM::evm_message_call_t(
                arith, sender, recipient, contract_address, gas, value,
...
__host__ __device__ int32_t CALLCODE(ArithEnv &arith,
    ...
    if (error_code == ERROR_SUCCESS) {
        ...
        new_state_ptr = new CuEVM::evm_call_state_t(
            ...
            new CuEVM::evm_message_call_t(
                arith, sender, recipient, contract_address, gas, value,
                current_state.message_ptr->get_depth() + 1, OP_CALLCODE,
                storage_address, call_data, code, ret_offset, ret_size,
                current_state.message_ptr->get_static_env()));
 ...
 * code.
 */
__host__ __device__ int32_t RETURN(ArithEnv &arith, const bn_t &gas_limit,
...
DELEGATECALL(ArithEnv &arith, CuEVM::evm_call_state_t &current_state,
    ...
    if (error_code == ERROR_SUCCESS) {
        ...
        CuEVM::evm_address_conversion(arith, address);
        bn_t sender;
        current_state.message_ptr->get_sender(
            arith, sender);  // keep the message call sender I_{s}
        bn_t recipient;
        ...
        new_state_ptr = new CuEVM::evm_call_state_t(
            ...
            new CuEVM::evm_message_call_t(
                arith, sender, recipient, contract_address, gas, value,
                current_state.message_ptr->get_depth() + 1, OP_DELEGATECALL,
                storage_address, call_data, code, ret_offset, ret_size,
                current_state.message_ptr->get_static_env()));
 ...
 * @param[out] return_data The return data.
 */
__host__ __device__ int32_t REVERT(ArithEnv &arith, const bn_t &gas_limit,
                                   bn_t &gas_used, CuEVM::evm_stack_t &stack,
                                   CuEVM::evm_memory_t &memory,
                                   CuEVM::evm_return_data_t &return_data) {
    bn_t memory_offset, length;
...
__host__ __device__ int32_t SELFDESTRUCT(
    ...
    if (message.get_static_env()) {
        ...
        if (error_code == ERROR_SUCCESS) {
            ...
            cgbn_add(arith.env, recipient_balance, recipient_balance,
                     sender_balance);
