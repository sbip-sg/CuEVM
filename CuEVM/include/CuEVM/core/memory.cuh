...
struct evm_memory_t {
     ...
     */
    __host__ __device__ evm_memory_t() : data(), size(0) { memory_cost = 0; }
     ...
     */
    __host__ __device__ void get_memory_cost(gas_t &cost) const;
     ...
     */
    __host__ __device__ int32_t allocate_pages(uint32_t new_size);
     ...
     * @return 0 if success, otherwise the error code.
     */
    __host__ __device__ int32_t get_last_offset(const evm_word_t &index, const evm_word_t &length,
     ...
     * @return 0 if success, otherwise the error code.
     */
    __host__ __device__ int32_t grow(const evm_word_t &index, const evm_word_t &length);
     ...
     * @return 0 if success, otherwise the error code.
     */
    __host__ __device__ int32_t get(const evm_word_t &index, const evm_word_t &length, CuEVM::byte_array_t &data);
     ...
     * @return 0 if success, otherwise the error code.
     */
    __host__ __device__ int32_t set(const CuEVM::byte_array_t &data, const evm_word_t &index, const evm_word_t &length);
};
/**
