...
struct evm_call_state_t {
     ...
     */
    __device__ evm_call_state_t(CuEVM::evm_call_state_t* parent, CuEVM::evm_message_call_t* shared_message_ptr,
                                CuEVM::evm_message_call_t_shadow* shadow_message_ptr = nullptr,
                                CuEVM::evm_word_t* shared_stack_ptr = nullptr);

    __device__ evm_call_state_t() {}

    __device__ evm_call_state_t(CuEVM::evm_call_state_t* other);
    /**
    ...
    __device__ int32_t update(evm_call_state_t& other);
};
...
struct cached_evm_call_state {
    ...
    __device__ cached_evm_call_state(evm_call_state_t* state);  // copy from state to cache
    __device__ cached_evm_call_state() {};
    __device__ void set_byte_code(const byte_array_t* byte_code);
    __device__ void write_cache_to_state(evm_call_state_t* state);  // copy from cache to state
};
}  // namespace CuEVM
