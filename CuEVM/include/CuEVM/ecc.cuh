...
inline constexpr const char *secp256k1_FieldPrime =
    "0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f";
inline constexpr const char *secp256k1_Order =
    "0xfffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141";

inline constexpr const char *secp256k1_GX =
    "0x79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798";
inline constexpr const char *secp256k1_GY =
    "0x483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8";

/// The alt_BN128 field prime number (P) and order
inline constexpr const char *alt_BN128_FieldPrime =
    "0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47";
inline constexpr const char *alt_BN128_Order =
    "0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001";

inline constexpr const char *alt_BN128_G2_B_0 =
    "0x2b149d40ceb8aaae81be18991be06ac3b5b4c5e559dbefa33267e6dc24a138e5";
inline constexpr const char *alt_BN128_G2_B_1 =
...
inline constexpr const char *alt_BN128_G2X1 =
    "0x1800deef121f1e76426a00665e5c4479674322d4f75edadd46debd5cd992f6ed";
inline constexpr const char *alt_BN128_G2X2 =
    "0x198e9393920d483a7260bfb731fb5d25f1aa493335a9e71297e485b7aef312c2";
inline constexpr const char *alt_BN128_G2Y1 =
    "0x12c85ea5db8c6deb4aab71808dcb408fe3d1e7690c43d37b4ce6cc0166fa7daa";
inline constexpr const char *alt_BN128_G2Y2 =
...
__host__ __device__ static const int32_t FQ12_mod_coeffs[] = {
...
template <size_t Degree>
__host__ __device__ __forceinline__ constexpr const int32_t *
get_modulus_coeffs();

template <>
__host__ __device__ __forceinline__ constexpr const int32_t *
get_modulus_coeffs<2>() {
    return FQ2_mod_coeffs;
}
template <>
__host__ __device__ __forceinline__ constexpr const int32_t *
get_modulus_coeffs<1>() {
 ...
 * @return Curve
 */
__host__ __device__ __forceinline__ Curve get_curve(ArithEnv &arith,
                                                    int curve_id) {
    Curve curve;
    if (curve_id == 256) {
...
__host__ __device__ __forceinline__ void cgbn_mul_mod(env_t env, bn_t &res,
                                                      bn_t &a, bn_t &b,
                                                      bn_t &mod) {
...
__host__ __device__ __forceinline__ void cgbn_add_mod(env_t env, bn_t &res,
                                                      bn_t &a, bn_t &b,
                                                      bn_t &mod) {
...
__host__ __device__ __forceinline__ void cgbn_sub_mod(env_t env, bn_t &res,
                                                      bn_t &a, bn_t &b,
                                                      bn_t &mod) {
    // if b > a then a - b + mod
 ...
 * @param mod
 */
__host__ __device__ __forceinline__ void cgbn_div_mod(env_t env, bn_t &res,
                                                      bn_t &a, bn_t &b,
                                                      bn_t &mod) {
 ...
 */
__host__ __device__ __forceinline__ bool is_on_cuve_simple(env_t env, bn_t &Px,
                                                           bn_t &Py, bn_t &mod,
                                                           uint32_t B) {
...
template <size_t Degree>
__host__ __device__ __forceinline__ bool FQP_equals(ArithEnv &arith,
                                                    FQ<Degree> &P1,
                                                    FQ<Degree> &P2) {
    for (size_t i = 0; i < Degree; i++) {
...
__host__ __device__ __forceinline__ int ec_add(ArithEnv &arith, Curve curve,
                                               bn_t &ResX, bn_t &ResY, bn_t &Px,
                                               bn_t &Py, bn_t &Qx, bn_t &Qy) {
    bn_t mod_fp;
    bn_t lambda, numerator, denominator, temp, x_r, y_r;
    evm_word_t scratch_pad;
    cgbn_load(arith.env, mod_fp, &curve.FP);
    // point at infinity
    if (cgbn_equals_ui32(arith.env, Px, 0) &&
        cgbn_equals_ui32(arith.env, Py, 0)) {
        if (is_on_cuve_simple(arith.env, Qx, Qy, mod_fp, curve.B)) {
            ...
            return -1;
    } else if (cgbn_equals_ui32(arith.env, Qx, 0) &&
               cgbn_equals_ui32(arith.env, Qy, 0)) {
...
__host__ __device__ __forceinline__ int ec_mul(ArithEnv &arith, Curve curve,
                                               bn_t &ResX, bn_t &ResY, bn_t &Gx,
                                               bn_t &Gy, bn_t &n) {
    bn_t mod_fp;
    evm_word_t scratch_pad;
    cgbn_load(arith.env, mod_fp, &curve.FP);
    ...
    if (cgbn_equals_ui32(arith.env, Gx, 0) &&
            cgbn_equals_ui32(arith.env, Gy, 0) ||
        cgbn_equals_ui32(arith.env, n, 0)) {
        cgbn_set_ui32(arith.env, ResX, 0);
        cgbn_set_ui32(arith.env, ResY, 0);
        return 0;
    }
    ...
    for (int i = bit_array_length - 2; i >= 0; --i) {
        // Gz = 2 * Gz
        ec_add(arith, curve, temp_ResX, temp_ResY, temp_ResX, temp_ResY,
               temp_ResX, temp_ResY);

        if (bitArray[CuEVM::word_bits - 1 - i]) {
            ec_add(arith, curve, temp_ResX, temp_ResY, temp_ResX, temp_ResY, Gx,
                   Gy);
        }
    }
...
__host__ __device__ __forceinline__ void convert_point_to_address(
    ArithEnv &arith, bn_t &address, bn_t &X, bn_t &Y) {
    evm_word_t scratch_pad;
    uint8_t input[64];
 ...
 * @param signer
 */
__host__ __device__ __forceinline__ int ec_recover(ArithEnv &arith,
                                                   signature_t sig,
                                                   bn_t &signer) {
    Curve curve;
    curve.FP.from_hex(secp256k1_FieldPrime);
    ...
    if (cgbn_equals_ui32(arith.env, temp_compare, 0) ||
        cgbn_compare(arith.env, r, mod_order) >= 0)
        return -1;
    ...
    if (cgbn_equals_ui32(arith.env, temp_compare, 0) ||
        cgbn_compare(arith.env, temp_cgbn, mod_order) >= 0)
        return -1;
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void getFQ12_from_cgbn_t(
    ArithEnv &arith, FQ<Degree> &res, bn_t (&coeffs)[Degree]) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_add(ArithEnv &arith,
                                                 FQ<Degree> &Res,
                                                 FQ<Degree> &P1, FQ<Degree> &P2,
                                                 bn_t &mod) {
    for (size_t i = 0; i < Degree; i++) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_sub(ArithEnv &arith,
                                                 FQ<Degree> &Res,
                                                 FQ<Degree> &P1, FQ<Degree> &P2,
                                                 bn_t &mod) {
    for (size_t i = 0; i < Degree; i++) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ uint deg(ArithEnv &arith,
                                             const FQ<Degree> &P) {
    uint res = Degree - 1;
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void poly_rounded_div(
    ArithEnv &arith, FQ<Degree> &Res, FQ<Degree> &A, FQ<Degree> &B, bn_t &mod) {
    ...
    for (int32_t i = dega - degb; i >= 0; i--) {
        ...
        cgbn_div_mod(arith.env, temp_res, temp.coeffs[degb + i], B.coeffs[degb],
                     mod);
        cgbn_add_mod(arith.env, o.coeffs[i], o.coeffs[i], temp_res, mod);
        for (size_t c = 0; c <= degb; c++) {
            cgbn_sub_mod(arith.env, temp.coeffs[c + i], temp.coeffs[c + i],
                         o.coeffs[c], mod);
        }
    }
    // return o[:deg(o)+1]
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_copy(ArithEnv &arith,
                                                  FQ<Degree> &Res,
                                                  FQ<Degree> &P) {
    for (size_t i = 0; i < Degree; i++) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_mul(ArithEnv &arith,
                                                 FQ<Degree> &Res,
                                                 FQ<Degree> &P1, FQ<Degree> &P2,
                                                 bn_t &mod) {
    if (Degree == 1) {
    ...
    for (size_t i = 0; i < Degree; i++) {
        if (mod_coeffs_array[i] < 0) {
            ...
            cgbn_sub_ui32(arith.env, mod_coeffs[i], mod,
                          0 - mod_coeffs_array[i]);
        } else {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_inv(ArithEnv &arith,
                                                 FQ<Degree> &Res, FQ<Degree> &P,
                                                 bn_t &mod) {
    if (Degree == 1) {
    ...
    while (deg(arith, low) >
           0) {  // Assuming a function `deg` to find the polynomial's degree
        FQ<Degree + 1> r;
        ...
        for (size_t i = 0; i <= Degree; ++i) {
            for (size_t j = 0; j <= Degree - i; ++j) {
                ...
                cgbn_sub_mod(arith.env, nm.coeffs[i + j], nm.coeffs[i + j],
                             temp, mod);

                // new[i+j] -= low[i] * r[j]
                cgbn_mul_mod(arith.env, temp, low.coeffs[i], r.coeffs[j], mod);
                cgbn_sub_mod(arith.env, new_h.coeffs[i + j],
                             new_h.coeffs[i + j], temp, mod);
            }
        }
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_div(ArithEnv &arith,
                                                 FQ<Degree> &Res,
                                                 FQ<Degree> &P1, FQ<Degree> &P2,
                                                 bn_t &mod) {  // P1/P2
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_neg(ArithEnv &arith,
                                                 FQ<Degree> &Res, FQ<Degree> &P,
                                                 bn_t &mod) {
    for (size_t i = 0; i < Degree; i++) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_pow(ArithEnv &arith,
                                                 FQ<Degree> &Res, FQ<Degree> &P,
                                                 bn_t &n, bn_t &mod) {
    if (Degree == 1) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_mul_scalar(ArithEnv &arith,
                                                        FQ<Degree> &Res,
                                                        FQ<Degree> &P, bn_t &n,
                                                        bn_t &mod) {
    for (size_t i = 0; i < Degree; i++) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ bool FQP_is_on_curve(
    ArithEnv &arith, FQ<Degree> &Px, FQ<Degree> &Py, bn_t &mod, FQ<Degree> &B) {
    // y^2 = x^3 + B
...
template <size_t Degree>
__host__ __device__ __forceinline__ bool FQP_is_valid(ArithEnv &arith,
                                                      FQ<Degree> &P,
                                                      bn_t &mod) {
    for (size_t i = 0; i < Degree; i++) {
...
template <size_t Degree>
__host__ __device__ __forceinline__ bool FQP_is_inf(ArithEnv &arith,
                                                    FQ<Degree> &Px,
                                                    FQ<Degree> &Py) {
    bool res = true;
    for (size_t i = 0; i < Degree; i++) {
        res = res && cgbn_equals_ui32(arith.env, Px.coeffs[i], 0) &&
              cgbn_equals_ui32(arith.env, Py.coeffs[i], 0);
    }
    return res;
}
/**
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_ec_add(
    ArithEnv &arith, FQ<Degree> &ResX, FQ<Degree> &ResY, FQ<Degree> &Px,
    FQ<Degree> &Py, FQ<Degree> &Qx, FQ<Degree> &Qy, bn_t &mod_fp) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_ec_mul(
    ArithEnv &arith, FQ<Degree> &ResX, FQ<Degree> &ResY, FQ<Degree> &Gx,
    FQ<Degree> &Gy, bn_t &n, bn_t &mod_fp) {
    uint8_t bitArray[CuEVM::word_bits];
    uint32_t bit_array_length = 0;
    ...
    for (int i = bit_array_length - 2; i >= 0; --i) {
        // Gz = 2 * Gz
        FQP_ec_add(arith, temp_ResX, temp_ResY, temp_ResX, temp_ResY, temp_ResX,
                   temp_ResY, mod_fp);
        if (bitArray[CuEVM::word_bits - 1 - i]) {
            // printf("bit1 add i %d\n", i);
            // Gz = Gz + P
            FQP_ec_add(arith, temp_ResX, temp_ResY, temp_ResX, temp_ResY, Gx,
                       Gy, mod_fp);
        }
    }
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_linefunc(
    ArithEnv &arith, FQ<Degree> &Res, FQ<Degree> &P1x, FQ<Degree> &P1y,
    FQ<Degree> &P2x, FQ<Degree> &P2y, FQ<Degree> &Tx, FQ<Degree> &Ty,
    bn_t &mod) {
 ...
 * @param mod_fp
 */
__host__ __device__ __forceinline__ void FQP_twist(ArithEnv &arith, FQ<12> &Rx,
                                                   FQ<12> &Ry, FQ<2> &Px,
                                                   FQ<2> &Py, bn_t &mod_fp) {
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void FQP_final_exponentiation(
    ArithEnv &arith, FQ<Degree> &res, FQ<Degree> &p, bn_t &mod) {
    evm_word_t temp_mem;
    ...
    for (int i = final_exp_len - 1; i >= 0; i--) {
        ...
        cgbn_load(arith.env, temp_n, &temp_mem);
 ...
 */
template <size_t Degree>
__host__ __device__ __forceinline__ void miller_loop(
    ArithEnv &arith, FQ<Degree> &Result, FQ<Degree> &Qx, FQ<Degree> &Qy,
 ...
 * @param final_exp if true, do the final exponentiation
 */
__host__ __device__ __forceinline__ void pairing(
    ArithEnv &arith, FQ<12> &Res, FQ<2> &Qx, FQ<2> &Qy, FQ<1> &Px, FQ<1> &Py,
    bn_t &mod_fp, bn_t &curve_order, bool final_exp = true) {
    ...
    miller_loop(arith, Res, Qx_tw, Qy_tw, Px_fp12, Py_fp12, mod_fp, curve_order,
                final_exp);
}
 ...
 *
 */
__host__ __device__ __forceinline__ int pairing_multiple(ArithEnv &arith,
                                                         uint8_t *points_data,
                                                         size_t data_len) {
    ...
    for (int i = 0; i < num_pairs; i++) {
        ...
        if (!on_curve || !valid) {
            ...
            if (FQP_is_inf(arith, Qx, Qy) || FQP_is_inf(arith, Px, Py)) {
                ...
                pairing(arith, temp_res, Qx, Qy, Px, Py, mod_fp, curve_order,
                        false);
        }
