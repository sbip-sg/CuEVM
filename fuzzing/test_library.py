...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_instances", type=int, default=1, help="number of instances"
    )
    ...
    if len(instance_data["transaction"]["data"]) < num_instances:
        instance_data["transaction"]["data"] = instance_data["transaction"]["data"] * (
            num_instances // len(instance_data["transaction"]["data"])
        )
    if len(instance_data["transaction"]["gasPrice"]) < num_instances:
        instance_data["transaction"]["gasPrice"] = instance_data["transaction"][
            "gasPrice"
        ] * (num_instances // len(instance_data["transaction"]["gasPrice"]))
    if len(instance_data["transaction"]["gasLimit"]) < num_instances:
        instance_data["transaction"]["gasLimit"] = instance_data["transaction"][
            "gasLimit"
        ] * (num_instances // len(instance_data["transaction"]["gasLimit"]))
    print(f'generated {len(instance_data["transaction"]["data"])} instances')

    process_json(instance_data, output_file)
